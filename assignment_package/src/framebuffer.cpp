...
//am i drawing the chunks to a texture?
//am i drawing the screen spanning quad?
//void paint(){
...
FrameBuffer::FrameBuffer(OpenGLContext *context,
                         unsigned int width, unsigned int height, float devicePixelRatio)
...
void FrameBuffer::resize(unsigned int width, unsigned int height, float devicePixelRatio) {
    m_width = width;
    m_height = height;
...
void FrameBuffer::createShadowMap() {
    ...
    int newWidth = m_width * m_devicePixelRatio; // scaleFactor > 1
    int newHeight = m_height * m_devicePixelRatio; // scaleFactor > 1

    mp_context->glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, newWidth, newHeight, 0, GL_RGB, GL_FLOAT, (void*)0);

    //mp_context->glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, m_width * m_devicePixelRatio, m_height * m_devicePixelRatio, 0, GL_RGB, GL_FLOAT, (void*)0);
