...
//    void jump() override;
    void toggleFlightMode() override;

    bool isOnGround();

    bool gridMarch(glm::vec3 rayOrigin, glm::vec3 rayDirection, const Terrain &terrain, float *out_dist, glm::ivec3 *out_blockHit, float *out_iAxis, glm::vec3 *out_iPos, BlockType *BT);

    void handleCollision(float dT, const Terrain &terrain, float &dX, float &dY, float &dZ);
