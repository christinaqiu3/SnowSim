...
private:
    ...
    std::unordered_set<Chunk*> m_freshlySpawnedChunks;
    QMutex m_freshlySpawnedChunksLock;
...
public:
    ...
    void setLightLevelAt(int x, int y, int z, int t);
    void removeBlockAt(int x, int y, int z);

    // Draws every Chunk that falls within the bounding box
    // described by the min and max coords, using the provided
    // ShaderProgram
    void draw(int minX, int maxX, int minZ, int maxZ, ShaderProgram *shaderProgram, bool drawTrans);
    ...
    void TEST();
    void createFirstTGZ();
    void redoFirstTGZ(float x, float z);
    void tickCreateChunk(float x, float z);
    void tickCreateChunk(float x, float z, float oldX, float oldZ);
    void spawnVBOWorker(Chunk* ch);
    void spawnBDWorker(int64_t tgz);
    void clearThreads();
